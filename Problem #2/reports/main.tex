...
\begin{itemize}[leftmargin=*]
  \item \textbf{Careers and institutions.} We study a STEM career (Software Developers; San Diego State University), a trade career (Electricians; Los Angeles Trade--Technical College), and an arts career (Writers and Authors; Academy of Art University). \textbf{National scenario results are reported for employment-weighted SOC bundles} (2--3 SOCs per career; Table~\ref{tab:scenario_summary} and Figure~\ref{fig:scenario_bar}), while institution context uses OEWS for the local labor market.
  ...
  \]
  This creates a substitution penalty for exposed occupations while bounding complementarity uplift for sheltered occupations by an occupation-level multiplier \(m_i\) (with conservative upper bound \(m_{\max}=0.2\)). See Table~\ref{tab:scenario_summary}.
  ...
  \item \textbf{Cheap credibility wins (independent checks).} To address the judge's ``is this plausible?'' instinct, we benchmark our index against Felten--Raj--Seamans AIOE: calibrated NetRisk correlates strongly (\(r=0.942\), \(\rho=0.938\); Table~\ref{tab:external_benchmark}) and we transparently list large-disagreement examples (Table~\ref{tab:external_benchmark_disagree}). We also audit our two NetRisk constructions (calibrated vs.\ equal-weight mechanism): overall correlation, sign agreement, and where they disagree most (Tables~\ref{tab:netrisk_index_compare}--\ref{tab:netrisk_index_disagree}).
...
% ============================================================
% 1b) Definitions & Provenance (one-page box)
% ============================================================
\section*{Definitions \& Provenance (for auditability)}
\begin{center}
